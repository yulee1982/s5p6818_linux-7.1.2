...
static int clk_syscore_suspend(void *data) { return 0; }

static void clk_syscore_resume(void *data) {}

static struct syscore_ops clk_syscore_ops = {
	.suspend = clk_syscore_suspend, .resume = clk_syscore_resume,
};

static struct syscore clk_syscore = {
	.ops	= &clk_syscore_ops,
};
