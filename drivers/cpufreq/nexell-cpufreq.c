...
static int nxp_cpufreq_verify_speed(struct cpufreq_policy_data *policy)
{
	struct cpufreq_dvfs_info *dvfs = get_dvfs_ptr();
...
static int nxp_cpufreq_set_target(struct cpufreq_policy *policy,
				unsigned int index)
{
	struct cpufreq_dvfs_info *dvfs = get_dvfs_ptr();
	struct cpufreq_frequency_table *freq_table = dvfs->freq_table;
	unsigned long rate_khz = 0;
	unsigned int old, new, new_min, new_max;
	int ret = 0;

	old = policy->cur;
	new = freq_table[index].frequency;

	new_max = freq_qos_read_value(&policy->constraints, FREQ_QOS_MAX);
	new_min = freq_qos_read_value(&policy->constraints, FREQ_QOS_MIN);
	if(new < new_min)
		new = new_min;
	if(new > new_max)
		new = new_max;
...
static void nxp_cpufreq_ready(struct cpufreq_policy *policy)
{
	...
	dev_pm_qos_add_notifier(&cdev->device, &cpufreq_min_qos_notifier, DEV_PM_QOS_MIN_FREQUENCY);
	dev_pm_qos_add_notifier(&cdev->device, &cpufreq_max_qos_notifier, DEV_PM_QOS_MAX_FREQUENCY);
}

static struct cpufreq_driver nxp_cpufreq_driver = {
	.flags   = CPUFREQ_NEED_INITIAL_FREQ_CHECK,
	.verify  = nxp_cpufreq_verify_speed,
	.target_index  = nxp_cpufreq_set_target,
