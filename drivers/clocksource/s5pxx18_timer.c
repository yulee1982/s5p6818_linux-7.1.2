 ...
 */
static u64 nexell_read_sched_clock(void)
{
	return timer_source_read(NULL);
}
static int __init timer_source_of_init(struct device_node *node)
{
	...
	sched_clock_register(nexell_read_sched_clock, 32, info->rate);
 ...
 */
u64 __arch_counter_get_cntvct(void) { return timer_read_count(); }
EXPORT_SYMBOL(__arch_counter_get_cntvct);

u64 arch_timer_read_counter(void) { return timer_read_count(); }
EXPORT_SYMBOL(arch_timer_read_counter);
...
static int __init timer_event_of_init(struct device_node *node)
{
	...
	ret = request_irq(info->interrupt, timer_event_handler, IRQF_TIMER, "Event Timer IRQ", dev);
	if (ret)
